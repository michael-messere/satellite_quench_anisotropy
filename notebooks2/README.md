...
1. **`01_generate_allcentral_catalogs.ipynb`** (Binder only): for TNG100 & TNG50 at z=0 and z=0.05,
   computes each satellite's azimuthal angle and 3D distance from its central, for **all centrals**
   (`M* > 1e7`) and satellites `M* > 1e7`, **no radius cut**. Writes to `../data2/<sim>/<z>/`.
